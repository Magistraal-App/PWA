<?php 
    ...
    function token_put($args = [], $token_id = null) {
        ...
        if(isset($token_id)) {
            file_put_contents(ROOT."/log.txt", '['.date('H:i:s').']'."[UPDATED] Login token {$token_id} updated:".json_encode(['access_token' => $access_token, 'access_token_expires' => $args['access_token_expires'], 'refresh_token' => $refresh_token])."\n", FILE_APPEND);
            ...
            file_put_contents(ROOT."/log.txt", '['.date('H:i:s').']'."[CREATED] Login token {$token_id} created:".json_encode(['token_expires' => $token_expires, 'tenant' => $args['tenant'], 'access_token' => $access_token, 'access_token_expires' => $args['access_token_expires'], 'refresh_token' => $refresh_token])."\n", FILE_APPEND);
    ...
    function token_get($token_id) {
        ...
        if(time() > $token_data['token_expires']) {
            file_put_contents(ROOT."/log.txt", '['.date('H:i:s').']'."[EXPIRED] Login token {$token_id} expired.\n", FILE_APPEND);
            // Delete old token
    ...
    function token_delete($token_id) {
        ...
        if($response) {
            file_put_contents(ROOT."/log.txt", '['.date('H:i:s').']'."[DELETED] Login token {$token_id} deleted.\n", FILE_APPEND);
        } else {
            file_put_contents(ROOT."/log.txt", '['.date('H:i:s').']'."[ERROR] Failed to delete login token {$token_id}: ".json_encode($response)."\n", FILE_APPEND);
        }
